...
fn player_encounter_checking(
    ...
    if keyboard.just_pressed(KeyCode::LControl) && encounter_query
        .iter()
        .any(|&transform| wall_collision_check(player_translation,transform.translation))
        {
            println!("Changing to dialog");
            create_fadeout(&mut commands,GameState::Dialog,&ascii);
        }
}
...
fn player_movement(
    ...
    wall_query: Query<&Transform, (Without<Player>, Without<Grass>, With<TileCollider>)>,
    keyboard: Res<Input<KeyCode>>,
    time: Res<Time>
){
...
fn spawn_player(mut commands:Commands, ascii:Res<AsciiSheet>){
    ...
    commands
        ...
        .insert(Player{speed: 20.0,just_moved: false})
        .id();
